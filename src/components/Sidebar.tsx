...
export function Sidebar({ input, setInput, targetAmount, setTargetAmount }: SidebarProps) {
  ...
  const removeHousingPlan = (index: number) => {
    handleChange('housingPlans', input.housingPlans.filter((_, i) => i !== index));
  };
    ...
    <div className="w-full lg:w-96 bg-white p-6 shadow-lg overflow-y-auto h-screen sticky top-0">
      ...
      <div className="space-y-8">
        ...
        <section>
          ...
          <div className="space-y-3">
            <InputGroup label="手取り月収 (万円)" value={input.monthlyIncome} onChange={v => handleChange('monthlyIncome', v)} />
        ...
        <section>
          <h3 className="font-bold text-gray-700 mb-3 border-b pb-1">退職後の仕事・再雇用</h3>
          <div className="space-y-4">
            {input.postRetirementJobs.map((job, i) => (
              <div key={i} className="bg-gray-50 p-3 rounded border border-gray-200 relative">
                ...
                <InputGroup label="月収 (万円)" value={job.monthlyIncome} onChange={v => updateJob(i, 'monthlyIncome', v)} className="mb-2" />
                <InputGroup label="退職金 (万円)" value={job.retirementBonus} onChange={v => updateJob(i, 'retirementBonus', v)} />
              </div>
            ))}
        ...
        <section>
          ...
          <InputGroup label="基本生活費 (住居・教育除く月額・万円)" value={input.monthlyLivingCost} onChange={v => handleChange('monthlyLivingCost', v)} />
        </section>

        {/* 5. Housing */}
        <section>
          <h3 className="font-bold text-gray-700 mb-3 border-b pb-1">住居設定</h3>
          <p className="text-xs text-gray-500 mb-3">これからの住居プランを順に追加してください。</p>
          <div className="space-y-4">
            {input.housingPlans.map((plan, i) => (
              <div key={i} className="bg-gray-50 p-3 rounded border border-gray-200 relative">
                ...
                <InputGroup label="住宅費 (月額・万円)" value={plan.cost} onChange={v => updateHousingPlan(i, 'cost', v)} className="mb-2" />
                ...
                {plan.duration !== 'infinite' && (
                  <InputGroup label="期間 (年)" value={plan.duration as number} onChange={v => updateHousingPlan(i, 'duration', v)} />
                )}
              </div>
            ))}
          ...
          <div className="space-y-4">
            {input.children.map((child, i) => (
              <div key={i} className="bg-gray-50 p-3 rounded border border-gray-200 relative">
                ...
                <InputGroup label="誕生時期 (何年後)" value={child.birthYearOffset} onChange={v => updateChild(i, 'birthYearOffset', v)} className="mb-2" />
                <InputGroup label="養育費 (22歳まで月額・万円)" value={child.monthlyChildcareCost} onChange={v => updateChild(i, 'monthlyChildcareCost', v)} className="mb-2" />
                <div>
          ...
          <div className="space-y-4">
            {input.oneTimeEvents.map((evt, i) => (
              <div key={i} className="bg-gray-50 p-3 rounded border border-gray-200 relative">
                ...
                <div className="grid grid-cols-2 gap-2 mb-2">
                    <InputGroup label="年齢" value={evt.age} onChange={v => updateEvent(i, 'age', v)} />
                    <InputGroup label="金額 (万円)" value={evt.amount} onChange={v => updateEvent(i, 'amount', v)} />
                </div>
