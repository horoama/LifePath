  ...
  return (
    <div className="w-full lg:w-80 bg-white p-6 shadow-lg overflow-y-auto h-screen sticky top-0">
      ...
      <div className="space-y-6">
        ...
        {/* 2. Income/Expense */}
        <section>
          <h3 className="font-semibold text-gray-700 mb-3 border-b pb-1">収支設定</h3>
          <div className="space-y-3">
            <InputGroup label="手取り月収 (世帯合計・万円)" value={input.monthlyIncome} onChange={v => handleChange('monthlyIncome', v)} />
        ...
        {/* 4. Life Events */}
        <section>
          <h3 className="font-semibold text-gray-700 mb-3 border-b pb-1">ライフイベント</h3>
          <div className="space-y-3">
            <InputGroup label="子供が生まれる時期 (何年後)" value={input.childBirthYearsFromNow} onChange={v => handleChange('childBirthYearsFromNow', v)} />
            <InputGroup label="育児による積立減額 (月額・万円)" value={input.childcareReduction} onChange={v => handleChange('childcareReduction', v)} />
            <div>
              <label className="block text-sm text-gray-600 mb-1">教育費パターン</label>
              <select
                value={input.educationPattern}
                onChange={(e) => handleChange('educationPattern', e.target.value)}
        ...
        {/* 5. Exit Strategy */}
        <section>
          <h3 className="font-semibold text-gray-700 mb-3 border-b pb-1">出口戦略</h3>
          <div className="space-y-3">
